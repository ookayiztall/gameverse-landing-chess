...
export const BoardComponent: FC<{
  ...
  const movingTo = useGameSettingsState((state) => state.movingTo)
  const setMovingTo = useGameSettingsState((state) => state.setMovingTo)
  ...
  useEffect(() => {
    if (!movingTo) return
    finishMovingPiece(movingTo.tile)
  }, [movingTo])
  ...
  const intensity = selected ? 0.35 : 0
  ...
  return (
    <group position={[-3.5, -0.5, -3.5]}>
      ...
      <pointLight intensity={intensity} color="red" position={[redLightPosition.x, 1, redLightPosition.y]} />
      {board.map((row, i) => {
